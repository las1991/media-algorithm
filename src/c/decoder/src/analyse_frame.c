...
int FindBasePframe(AnalyseFrameGlobalParams* params, const char* nal_buf, int len)
{
    ...
    if (nal_unit_type == 1 || nal_unit_type == 5 || nal_unit_type == 2 || nal_unit_type == 19){
        ...
        int frame_num = u(params->g_log2_max_frame_num_minus4 + 4, buf, &StartBit);
        ...
        if(params->g_frame_mbs_only_flag == 0){
        ...
        if(params->g_redundant_pic_cnt_present_flag >= 1){
        ...
        if(slice_type != I && slice_type != I1 && slice_type != SI && slice_type != SI1){
            ...
            if(ref_pic_list_reordering_flag_l0 == 1){
                int first_half = params->g_log2_max_frame_num_minus4 + 4 - (8 - frame_numStartBit % 8);
                if(first_half > 0){
...
static int analyze_pps(AnalyseFrameGlobalParams* params, const char* pps_buf, int pps_len)
{
    // Analyze PPS 
    unsigned int StartBit = 0;
    char* buf = pps_buf;
    ...
    if (nal_unit_type == 8){
        ...
        params->g_pic_order_present_flag = pic_order_present_flag;
        ...
        params->g_redundant_pic_cnt_present_flag = redundant_pic_cnt_present_flag;
...
static int analyze_sps(AnalyseFrameGlobalParams* params, const char* sps_buf, int sps_len)
{
    // Analyze SPS 
    unsigned int StartBit = 0;
    char* buf = sps_buf;
    ...
    if (nal_unit_type == 7){
        ...
        params->g_log2_max_frame_num_minus4 = log2_max_frame_num_minus4;
        int pic_order_cnt_type = Ue(buf, sps_len, &StartBit);
        params->g_pic_order_cnt_type = pic_order_cnt_type;
        ...
        if (pic_order_cnt_type == 0){
            ...
            params->g_delta_pic_order_always_zero_flag = delta_pic_order_always_zero_flag;
        ...
        params->g_frame_mbs_only_flag = frame_mbs_only_flag;
...
int AnalysePpsSps(AnalyseFrameGlobalParams* params, const char* pkt_buf, int pkt_size)
{
    if(params == NULL || pkt_buf == NULL || pkt_size <= 0)
        return -1;
    memset(params, 0, sizeof(AnalyseFrameGlobalParams));
    
    params->g_frame_mbs_only_flag              = -1;
    params->g_pic_order_cnt_type               = -1;
    params->g_delta_pic_order_always_zero_flag = -1;
    params->g_log2_max_frame_num_minus4        = -1;
    params->g_pic_order_present_flag           = -1;
    params->g_redundant_pic_cnt_present_flag   = -1;
    ...
    if(bSpsComplete)
        analyze_sps(params, sps_buf, sps_len);
    if(bPpsComplete)
        analyze_pps(params, pps_buf, pps_len);

    if(bSpsComplete == 0 && bPpsComplete == 0)
        return -1;
