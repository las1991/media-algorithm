                                     ...
                                     const uint8_t *buf, int len)
{
    ...
    // add by yujun
    pkt->packet_seq_missed = seq - s->seq - 1;
    if (pkt->packet_seq_missed == -RTP_SEQ_MOD) {
        pkt->packet_seq_missed = 0;
    }
    av_log(st ? st->codec : NULL, AV_LOG_DEBUG,
            "rtp sequence num = %d missed packet = %d\n",
            seq, pkt->packet_seq_missed);
...
static int rtp_parse_queued_packet(RTPDemuxContext *s, AVPacket *pkt)
{
    ...
    if (!has_next_packet(s)) {
        ...
        // add by yujun
        pkt->packet_seq_missed = s->queue->seq - s->seq -1;
    }
