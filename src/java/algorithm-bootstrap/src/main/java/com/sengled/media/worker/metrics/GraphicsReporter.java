...
public class GraphicsReporter extends ScheduledReporter {
    ...
    public static class Builder {
         ...
         */
        public GraphicsReporter build(MetricsGraphics metricsGraphics) {
            return new GraphicsReporter(registry,
                    metricsGraphics,
                    rateUnit,
                    durationUnit,
                    filter);
    ...
    private GraphicsReporter(MetricRegistry registry,MetricsGraphics metricsGraphics,
    ...
    private void logGauge(String name,
    	...
    	try{
    		Graphics graphics = getTable(prefix(name), "GAUGE", "value");
    		graphics.addRow(gauge.getValue());
    	} catch(Exception ex) {
    		ex.printStackTrace();
    	}
    }
    ...
    private Graphics getTable(String name,
                           String type,
                           String colTemplates) {
    	Graphics graphics = metricsGraphics.getOrCreateGraphics(name,
                type,
                colTemplates);
