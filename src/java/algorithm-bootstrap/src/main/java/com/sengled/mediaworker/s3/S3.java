...
public class S3 implements InitializingBean {
	...
	private void initialize() {
        ...
        if ( deploymentTarget.equals("local") ) {
            ...
            // 配置过期规则
            configExpiredTag(storageProperties.getExpired(), s3, bucketName);
        }
	...
	private void configExpiredTag(Map<String, String> expired, AmazonS3 s3Client, String bucketName) {
        ...
        for (Entry<String, String> entry : expired.entrySet()) {
            ...
            LifecycleFilter filter = new LifecycleFilter(
                    new LifecycleTagPredicate(new Tag(StorageProperties.EXPIRED_KEY, entry.getKey())));
            BucketLifecycleConfiguration.Rule rule = new BucketLifecycleConfiguration.Rule().withId(ruleName)
                    .withExpirationInDays(days).withFilter(filter)
