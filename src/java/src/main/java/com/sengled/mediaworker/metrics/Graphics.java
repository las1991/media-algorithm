...
public class Graphics {
    ...
    public Graphics(String name, String colTemplate) {
        this.name = name;
    ...
    public boolean ouput(OutputStream out, String col) throws IOException {
        ...
        // 只显示最近 60 分钟的数据
        long mixCreated = System.currentTimeMillis() - (1000 * 60 * 60);
        ...
        for (Row row : arrayList) {
            ...
            if (colValues[colIndex] instanceof Double) {
            	double doubleVaue = ((Number)colValues[colIndex]).doubleValue();
                writer.writeValue(doubleVaue);
            } else if (colValues[colIndex] instanceof Number) {
